...
class Trainer_CCG_TRADES(Trainer_base):
    def __init__(self, args, writer, attack_name, device, loss_function=torch.nn.CrossEntropyLoss()):
        super(Trainer_CCG_TRADES, self).__init__(args, writer, attack_name, device, loss_function)
        ...
        for epoch in range(0, self.args.max_epochs):
            ...
            for idx, (datas, labels) in enumerate(train_loader):
                ...
                loss_nat, loss_trades, adv_data = trades_loss(model=model, x_natural=data_pair, y=labels.repeat(2),
                                                              optimizer=opt, step_size=self.args.alpha,
                                                              epsilon=self.args.epsilon, perturb_steps=self.args.iters,
                                                              beta=self.args.trades_beta, distance='l_inf')
                ...
                if _iter % self.args.n_eval_step == 0:
                    ...
                    print(f'[TRAIN]-[{epoch}]/[{self.args.max_epochs}]-iter:{_iter}: lr:{opt.param_groups[0]["lr"]}\n'
                          ...
                          f'loss_adv:{loss_ce.item():.3f}\tloss_con:{loss_con.item():.3f}\t'
                          f'loss_trades:{loss_trades.item():.3f}\n')

                    if self.writer is not None:
                        self.writer.add_scalar('Train/Loss_adv', loss_ce.item(),
                                               epoch * len(train_loader) + idx)
                        self.writer.add_scalar('Train/Loss_con', loss_con.item(),
                                               epoch * len(train_loader) + idx)
